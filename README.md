...
1. Double-check that you are in the "Cell-Tracking" directory.
    ...
    - You should get a file path ending in ```/Cell-Tracking```. For example, you should get something that looks like this: ```/Users/carolinetracy/Desktop/Cell-Tracking```.
      - If you don't get a file path ending in ```/Cell-Tracking```, you must find the path for the Cell-Tracking folder. Follow the **How To Get Cell-Tracking's Path** steps below. The path you just copied will be referred to as CELL-TRACKING-PATH.
2. Run ```cd CELL-TRACKING-PATH```
3. Run ```python3 -m venv venv```
   - Note: if you have python (not python3) installed, run ```python -m venv venv``` instead.
4. For Mac users: run ```source venv/bin/activate```. For Windows users: run ```venv\Scripts\activate```.
5. Run ```pip install pytest```
   - Note that this will take a second to install.
6. Find the file whose tests you want to run. This file will be referred to as TEST_FILE. (Make sure TEST_FILE includes ".py" at the end. For example, TEST_FILE could be tiffclass_test.py).
7. Figure out which folder TEST_FILE lies in. Note that this folder is a folder within the "tests" folder. This folder will be referred to as TEST_FOLDER.
...
8. Run ```pytest /TEST_FOLDER/TEST_FILE```
