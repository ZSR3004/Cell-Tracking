...
def test_vector_magnitude_heatmaps(init_tiff: tuple):
    ...
    def test_case_x(flowx: np.ndarray, normalizex: bool):
        ...
        """
        with (
            patch("numpy.linalg.norm") as mock_linalg_norm,
            ...
            if normalizex:
                ...
                for frame in fake_flow:
                    assert np.array_equal(
                        mock_cv2_normalize.call_args_list[i][0][0], frame
                    )
...
def test_save_heatmap_video(init_tiff: tuple, tmp_path):
    ...
    def test_case_x(flowx: np.ndarray, output_pathx: str, fpsx: int, normalizex: bool):
        ...
        with (
            patch(
                "src.cell_tracking.heatmap.vector_magnitude_heatmaps"
            ) as mock_vector_mag_heatmaps,
            ...
            patch(
                "src.cell_tracking.heatmap.animation.FuncAnimation"
            ...
            heatmap.save_heatmap_video(
                flowx, output_pathx, fps=fpsx, normalize=normalizex
            )

            args_vector_mag_heatmap, kwargs_vector_mag_heatmap = (
                mock_vector_mag_heatmaps.call_args
            )
            ...
            mock_ani.save.assert_called_once_with(
                output_pathx, fps=fpsx, extra_args=["-vcodec", "libx264"]
            )
